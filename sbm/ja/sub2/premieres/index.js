...
function make_table(data){
	...
	,y,mo,da,h,mi,title,channel,type,type_text,url,url_a,url_text
	...
	for(let i=0;i<data.length;i++){
		row_i=table.insertRow(-1);
			...
			try{//次の項目と同一時刻ならば続く行と結合
				if(time_next-time_i==0&&time_i-time_before!=0){
					time_rowspan=0;
